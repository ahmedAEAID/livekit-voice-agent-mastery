"""Lesson: execute multiple conversational tasks as a resumable group."""
...
class CommuteTask(AgentTask[CommuteResults]):
    ...
    def _check_completion(self):
        if self._results.get("commute_can") is False:
            self.complete(CommuteResults(commute_can=False, commute_method=""))
        elif self._results.keys() == {"commute_can", "commute_method"}:
            self.complete(
                CommuteResults(commute_can=True, commute_method=self._results["commute_method"])
            )
...
class HiringAgent(Agent):
    def __init__(self):
        ...
        super().__init__(
            instructions="You are a professional hiring coordinator.",
    ...
    async def on_enter(self) -> None:
        ...
        logger.info(
            f"✅ Workflow Complete: {intro_data.name} | Commute: {commute_data.commute_can}"
        )
