            ...
            api.RoomConfiguration(
                agents=[
                    api.RoomAgentDispatch(
                        ...
                        metadata=json.dumps({"user_id": "12345"})
                    )
                ],
            ),
        )
...
def register_rpc_methods(room: rtc.Room):
    ...
    async def on_confirmation(data: rtc.RpcInvocationData):
        ...
        print(f"⏳ Waiting 5 seconds before returning response (Timeout allowed: {data.response_timeout}s)...")
...
async def connect_participant(identity: str, room_name: str) -> rtc.Room:
    ...
    print(f"🔑 Generated Token for {identity}, \n token: {token}")
...
async def main():
    ...
    try:
        ...
        for _ in range(10):  # Check for 10 seconds
            if any(p.kind == rtc.ParticipantKind.PARTICIPANT_KIND_AGENT for p in ui_mock_room.remote_participants.values()):
                agent_joined = True
