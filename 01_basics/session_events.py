"""Lesson 4: observe state changes, transcripts, tools, and session shutdown."""
...
async def my_agent(ctx: agents.JobContext):
    ...
    session = create_session(user_away_timeout=10.0)
    ...
    def on_user_state_changed(event: UserStateChangedEvent):
        ...
        # 3. User is Away (Timeout hit)
            ...
            logger.warning("⚠️ User Away. (No input for 10s)")
    ...
    def on_function_tools_executed(event: FunctionToolsExecutedEvent):
        ...
        logger.info(
            f"🔧 Tool Event Flags: Reply Required={event.has_tool_reply}, Handoff Required={event.has_agent_handoff}"
        )
    ...
    def on_close(event: CloseEvent):
        ...
        # (Normal Hangup)
            ...
            logger.info("✅ Normal Closure. User hung up or room ended.")
